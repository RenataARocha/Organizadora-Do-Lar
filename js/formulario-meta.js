...
// 🗂️ RENDERIZAÇÃO DAS METAS
function carregarMetas() {
  listaMetas.innerHTML = '';
  const metas = pegarMetasStorage();

  if (metas.length === 0) {
  mensagemVazia.style.display = 'block';  // mostra a frase
} else {
  mensagemVazia.style.display = 'none';   // esconde a frase
}


  // Se tem metas, escolhe uma frase inspiradora aleatória e mostra
  metaInspiradora.textContent = metasInspiradoras[Math.floor(Math.random() * metasInspiradoras.length)];
  ...
  <div class="flex justify-between items-start gap-4 p-4 rounded-lg shadow bg-pink-50">
    <div class="flex-1 space-y-1 text-base font-semibold">
      <p>
        <strong class="text-pink-500">${meta.titulo}</strong>
      </p>
      <p class="text-black">${meta.descricao}</p>
      <p>
        <span class="text-pink-500">Categoria:</span>
        <span class="text-black">${meta.categoria}</span>
      </p>
      <p>
        <span class="text-pink-500">Prazo:</span>
        <span class="text-black">${meta.prazo || 'Não definido'}</span>
      </p>
      <p>
        <span class="text-pink-500">Prioridade:</span>
        <span class="text-black">${meta.prioridade}</span>
      </p>
      <p>
        <span class="text-pink-500">🔔 Lembrete:</span>
        <span class="text-black">${meta.lembrete}</span>
      </p>
    </div>
    <button 
